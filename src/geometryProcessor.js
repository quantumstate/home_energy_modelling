...
import { STOREY_LABELS, DEFAULT_U_VALUES } from "./constants.js";
 ...
 * The outward normal points away from the room centroid.
 *
 * @param {Vec2}    ptA
 * @param {Vec2}    ptB
 ...
 */
export function wallOutwardBearing(ptA, ptB, centroid, buildingRotation = 0) {
  ...
  for (const storey of model.storeys) {
    ...
    for (const room of storey.rooms) {
      ...
      for (let wi = 0; wi < nPts; wi++) {
        ...
        // Orientation
        const bearing     = wallOutwardBearing(ptA, ptB, cen, model.site.buildingRotation);
        ...
        const wallHLC      = adj.type === "external" ? uValue * netOpaqueArea : 0;
      ...
      const floorAdj  = si === 0 ? "ground" : "internal-heated";
